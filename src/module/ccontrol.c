 ...
 *
 * Colored devices are short-lived where as the control one lives as long as the
 *module.
 * Pages allocated to the module are saved in global memory (concurrent access
 *are not handled
 * for now.
...
/* Allocated Pages:
 * the kernel module reserves physical memory by making BIG allocations (BIG
 *enough
 ...
 *
 * Once reserved, an head is split into pages and a list of all the pages of the
 *same
 * color is saved into a global array.
 *
 * Heads need to be saved independently from colors because all heads do not
 *start
 * at the same color.
 */
...
int colored_vma_fault(struct vm_area_struct *vma, struct vm_fault *vmf) {
  ...
  if (offset >= dev->nbpages) {
    printk(KERN_ERR
           "ccontrol: dev: %u, offset %lu greater than device size %u.\n",
...
static struct file_operations colored_fops = {
    .owner = THIS_MODULE, .open = colored_open, .mmap = colored_mmap,
};
...
int create_colored(struct colored_dev **dev, color_set cset, size_t size) {
  ...
  *dev = kmalloc(sizeof(struct colored_dev), GFP_KERNEL);
  ...
  if ((*dev)->pages == NULL) {
    printk(
        KERN_ERR
        "ccontrol: vmalloc failed in create_colored, asked %zu page pointers\n",
        size);
    goto free_dev;
  }
...
static int ioctl_new(ioctl_args *arg) {
  ...
  if (bitmap_full(devmap, MAX_DEVICES)) return -ENOMEM;
...
#else
int control_ioctl(struct inode *inode, struct file *filp, unsigned int code,
                  unsigned long val)
#endif
{
...
void clean_pagetable(void) {
  int i;
  if (heads) kfree((void *)heads);

  if (pages) {
    for (i = 0; i < colors; i++)
      if (pages[i] != NULL) vfree((void *)pages[i]);
    kfree((void *)pages);
  }
  if (nbpages) kfree((void *)nbpages);
}
...
int reserve_memory(unsigned int nbh) {
  ...
  for (i = 0; i < nbh; i++) {
    ...
    if (!page) {
      printk(KERN_INFO "ccontrol: failed to get a page");
      return -ENOMEM;
    }
    heads[nbheads++] = page;
...
void free_memory(void) {
  int i;
  if (heads)
    for (i = 0; i < nbheads; i++) __free_pages(heads[i], order);
}
...
static int __init init(void) {
  ...
  if (blocks * (PAGE_SIZE * (1 << order)) < memory) blocks++;
